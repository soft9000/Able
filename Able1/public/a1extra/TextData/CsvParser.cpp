#include <extra/TextData/CsvParser.hpp>
//#include <presto.hpp>
...
void CsvParser::NORMALIZE_COMMAS(ZStr& std, size_t sz_start)
   {
...
void CsvParser::PUSH_LITERAL_QUOTES(ZStr& std, size_t sz_start)
   {
...
void CsvParser::POP_LITERAL_QUOTES(ZStr& std)
   {
...
void CsvParser::PUSH_LITERAL_COMMAS(ZStr& std, size_t sz_start)
   {
...
void CsvParser::POP_LITERAL_COMMAS(ZStr& std)
   {
...
void CsvParser::ConvertToTight(ZStr& result)
	{
   size_t ss;
   ss = result.Find("\", \"");
   while(ss != NPOS)
...
istream& CsvParser::GetField(istream& in, ZStr& result)
	{
   // BY THIS TIME, ANY LITERAL COMMAS AND QUOTES HAVE BEEN ENCODED;
   long top = in.tellg();

	result.ReadToDelim(in, ',');
...
istream& CsvParser::ImportCsvData(istream& in, ZStr& result)
	{
	result = "";
...
size_t CsvParser::GetLine(istream& ifs, Array<ZStr>& result, CsvDataType type)
   {
   result.Empty();
   if(ifs)
      {
...
size_t CsvParser::GetLine(const ZStr& sInput, Array<ZStr>& result, CsvDataType type)
   {
   result.Empty();
   ZStr str = sInput;
   if(type == StringsOnly)
...
size_t CsvParser::GetLine(Array<ZStr>& result, CsvDataType type)
   {
   result.Empty();
   if(pIfs && *pIfs)
      {
...
size_t CsvParser::GetLineStrings(istream& ifs, Array<ZStr>& result)
   {
   return GetLine(ifs, result, StringsOnly);
   }

size_t CsvParser::GetLineStrings(const ZStr& std,Array<ZStr>& result)
   {
   return GetLine(std, result, StringsOnly);
   }

size_t CsvParser::GetLineStrings(Array<ZStr>& result)
   {
