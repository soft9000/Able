...
StdString& StdString::assign(const string& str)
{
...
StdString& StdString::remove(char ch)
{
   stringstream srm;
...
StdString& StdString::remove(int start)
{
...
StdString& StdString::assign(char ch)
{
   char buf[5];
   ::sprintf(&buf[0], "%c", ch);
   return assign(&buf[0]);
...
StdString&  StdString::append(char ch, size_t count)
{
...
StdString& StdString::operator=(const string& str)
{
   return assign(str.c_str());
}
...
size_t StdString::find_any(char ch, size_t pos) const
{
return (find(ch, pos, 1));
}


size_t StdString::rfind_any(char ch, size_t pos) const
{
...
size_t StdString::find_last(char ch, size_t pos, int ignore_case) const
{
StdString sz = ch;
...
bool StdString::split(const StdString& ch, Array<StdString>& array) const
   {
   array.Empty();
   Array<ZStr> array2;
   PipeArray::Parse(array2, ZStr(this->c_str()), ZStr(ch.c_str()));
   if(array2.Nelem())
      {
      array.AddZombies(array2.Nelem());
      for(size_t ss = 0L; ss < array2.Nelem(); ss++)
         array.Append(array2[ss].c_str());
      return true;
      }
   return false;
   }
...
bool StdString::split(char ch, Array<StdString>& array) const
   {
   array.Empty();
   Array<ZStr> array2;
   array2.AddZombies(array.Nelem());
   for(size_t ss = 0L; ss < array.Nelem(); ss++)
      array2.Append(array[ss].c_str());
   PipeArray::Parse(array2, ZStr(this->c_str()), ch);
   if(array.Nelem())
      return true;
   return false;
   }
...
StdString  StdString::SetComma(int nelem)
   {
   StdString str;
   str.assign(nelem);
   return SetComma(str);
...
StdString  StdString::SetComma(const StdString& str)
   {
   StdString sHold = str;
   sHold.reverse();
   StdString sResult;
...
size_t stdnoj::Strip(StdString& str, char ch)
   {
   int sp = NULL;
   if(str.is_null())
      return sp;
...
size_t stdnoj::Clip(StdString& str, char ch)
   {
...
size_t stdnoj::Snip(const StdString& str, StdString& sLeft, StdString& sRight, char ch)
   {
   StdString sss = str;
...
int stdnoj::getline(istream& is, StdString& str)
   {
   str.read_line(is);
   if(is.good())
         return 1;
...
size_t stdnoj::rfind(const StdString& sSrc, const StdString& sPat, int ignore_case)
{
StdString str = sSrc;
str.reverse();
StdString spat = sPat;
...
StdString stdnoj::tag_remove(const StdString& sSrc, char chOn, char chOff)
{
...
StdString stdnoj::Common(const StdString& sC, const StdString& sD)  // Returns what to string have in common
{
StdString sResult;
...
StdString stdnoj::rCommon(const StdString& s1, const StdString& s2) // Returns what the END of two strings have in common
{
   StdString sA = s1;
   StdString sB = s2;
   sA.reverse();
   sB.reverse();
   StdString sC = stdnoj::Common(sA, sB);
   sC.reverse();
   return sC;
}


void stdnoj::Clone(Array<StdString>& array1, Array<StreamString>& array2)
{
   array2.Empty();
...
void stdnoj::Clone(Array<StreamString>& array1, Array<StdString>& array2)
{
   array2.Empty();
   array2.AddZombies(array1.Nelem());
   for(size_t ss = 0; ss < array1.Nelem(); ss++)
...
wstring stdnoj::convert(const string& str)
   {
...
string stdnoj::convert(const wstring& wstr)
   {
...
StdString
stdnoj::operator + (const StdString& str, const StdString& cstr)
   {
...
StdString&
stdnoj::operator += (StdString& str, const StdString& cstr)
   {
   str.append(cstr);
   return str;
   }

StdString& stdnoj::operator >>(StdString& str, size_t& sz)
   {
   sz = ::atoi(str.c_str());
   return str;
   }
...
StdString& stdnoj::operator <<(StdString& str, const StdString& str2)
   {
   str.append(str2);
   return str;
   }
...
StdString& stdnoj::operator <<(StdString& str, const char *psz)
   {
   str.append(psz);
   return str;
   }

istream& stdnoj::operator >> (istream& is, StdString& ss)
   {
   ss.read(is);
   return is;
   }
...
ostream& stdnoj::operator << (ostream& os, const StdString& ss)
   {
   os << ss.c_str();
   return os;
   }
