...
bool Socket::Connect(const ZStr& sHostName, u_short service_port, int af_which)
   {
   if(connected)
      Disconnect();
...
bool Socket::Send(const ZStr& std)
   {
...
bool Socket::Transact(const ZStr& in, ZStr& out)
   {
...
const char *Socket::GetError(ZStr& sLastError, int iCode) const
   {
