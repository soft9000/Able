...
PortTime::PortTime(void)                      {::memset(&goo, 0, sizeof(goo));}

PortTime::~PortTime(void)             {}

int PortTime::Day(void)              const    {return goo.DD;}
int PortTime::Month(void)            const    {return goo.MM;}
int PortTime::Year(void)             const    {return goo.YY;}

int PortTime::Hour24(void)           const    {return goo.HH;}
int PortTime::Minute(void)           const    {return goo.Mn;}
int PortTime::Second(void)           const    {return goo.SS;}

void PortTime::DateYMD(int yy, int mm, int dd)   {goo.MM = mm; goo.DD = dd; goo.YY = yy;}
void PortTime::TimeHMS(int hh, int mm, int ss)   {goo.HH = hh; goo.Mn = mm; goo.SS = ss;}

void PortTime::EncodeDate(int yy, int mm, int dd)   {DateYMD(yy, mm, dd);}
// DO NOT USE REFERENCES HERE: Causes problem under C++ Builder 5.x;
void PortTime::DecodeDate(int *yy, int *mm, int *dd){*yy = goo.YY; *mm = goo.MM; *dd = goo.DD;}

void PortTime::EncodeTime(int hh, int mm, int ss)   {TimeHMS(hh, mm, ss);}
// DO NOT USE REFERENCES HERE: Causes problem under C++ Builder 5.x;
void PortTime::DecodeTime(int *hh, int *mm, int *ss){*hh = goo.HH; *mm = goo.Mn; *ss = goo.SS;}

PortTime& PortTime::operator = (const PortTime& pt)
   {
   if(this == &pt)
      return *this;
   goo.MM = pt.goo.MM; goo.DD = pt.goo.DD; goo.YY = pt.goo.YY;
...
const char *PortTime::Format(ZStr& str, const char *pFmt) const
   {
   if(!pFmt)
      pFmt = sdtMasks::AsMDY();
