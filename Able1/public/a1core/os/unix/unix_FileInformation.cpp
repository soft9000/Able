...
bool FileInformation::Update(void)
   {
   ZStr sHold = Name();     // so `self` mess won't happen...
   return Get(sHold);
   }

bool FileInformation::Get(const ZStr& std)
   {
/*
...
bool FileInformation::Put(const ZStr& std)
   {
/*
...
istream& FileInformation::Read(istream& is)
   ...
   if(!is)
      return is;
   sFileName.ReadStream(is);
//   is.Read((BYTE *)&ofData, sizeof(BY_HANDLE_FILE_INFORMATION));
   return is;
   }


const char *FileInformation::CreationTime(ZStr& str)
   {
   return 0L;//FormatTime(ofData.ftCreationTime, str);
   }


const char *FileInformation::WriteTime(ZStr& str)
   {
   return 0L;//FormatTime(ofData.ftLastWriteTime, str);
   }


const char *FileInformation::ReadTime(ZStr& str)
   {
...
const char *FileInformation::FormatTime(const FILETIME& ff, ZStr& sResult)
   {
