...
#include <Able1.hpp>
#include <obsolete/Sockets/StdSocket.hpp>
...
struct ServerType
{
   ...
   int smtp(void)    {return 1;}
   int ftp(void)     {return 2;}
   int pop3(void)    {return 3;}
   int nntp(void)    {return 4;}
...
class HostName
   ...
   public:
      ...
      void        Name(const ZStr& std)          {sHost = std;}
...
class UserData
   ...
   public:
      ...
      const char *User(void)                          {return sUserId.c_str();}
      const char *Pass(void)                          {return sPassword.c_str();}
...
class LowLevelSocketStatusProbe : public StdLog
{
   ...
   public:
      ...
      ZStr Print(ostream& os, const ZStr& psz, int LogCapture = 1);
      ZStr PrintLine(ostream& os, const ZStr& psz, int LogCapture = 1);
};
...
class RfcObjectStatus
   ...
   public:
      ...
      RfcObjectStatus(void)                      {sLogFileName = "StatusObject.Log"; verbose = false;}
      virtual ~RfcObjectStatus(void)             {}

      void Use(const StdLog& rcLog)              {sLogFileName = rcLog.Name();}
      void Query(StdLog& rcLog)                  {rcLog.Name(sLogFileName.c_str());}

      StdLog *GetProbe(void)                  {return (new DayLog);} // LowLevelSocketStatusProbe(this));}
...
//    virtual void OnWithdrawn(const NewsGroup& group, ArticleThunk& thunk, int RfcCode, const ZStr& sBuffer);
      ...
      virtual void OnError(const EMail& group, const ZStr& RfcCode);

      // Added 01/08/1999
      void SetVerbose(bool b = true)          {verbose = b;}
      bool QueryVerbose(void)                 {return verbose;}
