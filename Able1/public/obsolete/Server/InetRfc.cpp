...
bool Rfc822Server::Put(const ZStr& std)
   {
   return soc.Send(std, l_timeout);
   }
bool Rfc822Server::Get(ZStr& std)
   {
...
bool Rfc822Server::TransactCodeLevel(const ZStr& std, ZStr& sResult, int RFC_code)
   {
...
bool Rfc822Server::TransactCodeSpecific(const ZStr& std, ZStr& sResult, int RFC_code)
   {
   if(Transact(std, sResult) == false)
      return false;
   RfcStatusCode RfcCode;
...
void Rfc822Server::LastError(ZStr& std)
   {
...
bool Rfc977NewsGroupServer::Post(const EMail& email, ZStr& sError)
   {
   EMail em = email;
   bool br = false;
   sError = "POST: ";
...
bool SmtpServer::Post(const OutBoundEmail& obm, ZStr& sError)
   {
   EMail emOut;
   emOut = obm;
   emOut._to      = obm.To();
...
bool SmtpServer::Post(const EMail& email, ZStr& sError)
   {
   ZStr std, sResult;
   std = "MAIL FROM:<";
...
bool Pop3Server::GetMagicBuffer(size_t which, ZStr& sResult)
   ...
   sz = std.Find("\r\n");
   if(sz != NPOS)
	  std.RemovePos(0, sz+2);      // Chomp the RETR message
   sResult = std; 
...
void Pop3Batch::Error(const ZStr& std)
   {
...
void Pop3Batch::RecallErrors(ZStr& std)
   {
   std = "";
